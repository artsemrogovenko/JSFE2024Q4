class Game {
  ...
  generateSymbols() {
    ...
    switch (this.mode) {
      case 'easy':
        this.required = this.required.map(() => randomElement(this.digits));
        break;
      case 'medium':
        this.required = this.required.map(() => randomElement(this.alphabet));
        break;
      case 'hard':
        this.required = this.required.map(() => randomElement(this.mixed));
        break;
      default:
        break;
    }
  ...
  set validateInput(symbol) {
    ...
    if (this.gaming && this.inputAvailable) {
      ...
      if (this.dictionary[this.mode].includes(symbol) && !this.goNext) {
        ...
        if (
          (this.mistakes === 2 && !this.goNext) ||
          (this.mistakes === 1 && !this.goNext && this.helped)
        ) {
          this.gameOver();
          return;
        }
