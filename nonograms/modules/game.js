...
export default class Game {
  ...
  constructor(size = 5) {
    ...
    this.#selectorDifficulty.addEventListener('change', (event) =>
      this.#selectDifficulty(event.target),
    );
    this.#selectorNonograms.addEventListener('change', () =>
      this.#selectNonogram(),
    );
  ...
  #selectNonogram() {
    ...
    this.resetCells();
    this.#difficulty =
      this.#selectorDifficulty.options[
        this.#selectorDifficulty.selectedIndex
      ].text;
    this.#nonogram =
      this.#selectorNonograms.options[
    ...
    const [countsLeft, countsTop, combination] =
      this.#template.calculateDigitForTip(this.#currentSize);
  ...
  #selectImage(imgName) {
    rootStyle.setProperty(
      '--image-src',
      `url(../assets/images/${imgName}.jpg?v=2)`,
    );
  }
  ...
  handleState(event) {
    ...
    if (this.#userInput.dark.size === this.#winnerCombination.length) {
      let isWinner = this.#template.calculateWinnerCombination(
      ...
      if (isWinner) {
        ...
        this.#gameLayout.popUps.greetMsg(this.#difficulty, this.#nonogram, [
          minutes,
          seconds,
        ]);
